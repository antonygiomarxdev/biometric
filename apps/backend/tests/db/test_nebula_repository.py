...
def _make_result(is_succeeded: bool = True, row_size: int = 0, rows: list | None = None) -> MagicMock:
...
def _make_value(as_string: str = "", as_int: int = 0, as_float: float = 0.0, as_bool: bool = False) -> MagicMock:
    val = MagicMock()
...
def _make_vertex_row(vid: str, node_idx: int, degree: int, x: int, y: int, weight: float, is_cutoff: bool) -> MagicMock:
    row = MagicMock()
    row.values = [
...
@pytest.fixture
def repo() -> NebulaRepository:
    """A NebulaRepository backed by a mock pool that returns mock sessions."""
    mock_pool = MagicMock()
...
class TestEnsureSpace:
    def test_creates_space_tags_and_index(self, repo: NebulaRepository) -> None:
        session = repo._get_session()
        session.execute.return_value = _make_result(is_succeeded=True)

        repo.ensure_space()

        executed_queries = [
            call[0][0] for call in session.execute.call_args_list
        ]
...
class TestInsertGraph:
    def test_inserts_vertices_and_edges(self, repo: NebulaRepository, tiny_graph: RidgeGraph) -> None:
        session = repo._get_session()
        session.execute.return_value = _make_result(is_succeeded=True)

        repo.insert_graph("fp99", tiny_graph)

        queries = [call[0][0] for call in session.execute.call_args_list]
    ...
    def test_empty_graph(self, repo: NebulaRepository) -> None:
        session = repo._get_session()
        session.execute.return_value = _make_result(is_succeeded=True)
...
class TestMatchSubgraph:
    def test_returns_matching_candidates(self, repo: NebulaRepository) -> None:
        ...
        edge_ok = _make_result(
            is_succeeded=True, row_size=1,
            rows=[_make_edge_row("c1:1", 10)],
        )

        session = repo._get_session()
        # Each candidate triggers: USE (skipped if already), LOOKUP, GO per vertex
        def side_effect(query: str) -> MagicMock:
            if "LOOKUP ON" in query:
                if "c1" in query or "cand1" in query:
                    return lookup_ok
    ...
    def test_no_candidates_returns_empty(self, repo: NebulaRepository) -> None:
        latent = RidgeGraph(
            nodes=[RidgeNode(x=0, y=0), RidgeNode(x=1, y=1)],
            edges=[RidgeEdge(source=0, target=1, path=[], length=10)],
        )
        session = repo._get_session()
        session.execute.return_value = _make_result(is_succeeded=True, row_size=0)
        results = repo.match_subgraph(latent, candidate_ids=[], top_k=5)
        assert results == []


class TestLoadGraph:
    def test_loads_graph_from_lookup(self, repo: NebulaRepository) -> None:
        lookup_ok = _make_result(
            is_succeeded=True, row_size=2,
            rows=[
        ...
        G = repo._load_graph(session, "fp98")
    ...
    def test_no_vertices_returns_none(self, repo: NebulaRepository) -> None:
        session = repo._get_session()
        session.execute.return_value = _make_result(is_succeeded=True, row_size=0)

        G = repo._load_graph(session, "nonexistent")
        assert G is None
