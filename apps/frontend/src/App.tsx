    ...
    try {
      ...
      if (extractRes.minutiae && extractRes.minutiae.length > 0) {
        ...
        addToast({
          ...
          description: "La imagen podría no ser válida o estar demasiado borrosa",
        });
      }
      ...
      if (action === "identify") {
        ...
        if (res.matched) {
          ...
          addToast({
            ...
            description: "La huella no coincide con ningún registro en la base de datos",
            duration: 4000,
          });
        }
      } else {
      ...
      let errorDescription = "Ocurrió un error inesperado. Por favor, intenta nuevamente.";
      ...
      if (error instanceof ApiError) {
        ...
        if (error.status === 0 || error.status >= 500) {
          ...
          errorDescription = "El servidor no está respondiendo. Verifica que el backend esté corriendo.";
        } else if (error.status === 400) {
          errorMessage = "Solicitud inválida";
          errorDescription = error.body?.detail || "Los datos proporcionados no son válidos.";
        } else if (error.status === 404) {
          errorMessage = "Recurso no encontrado";
          errorDescription = "El endpoint solicitado no existe.";
        } else if (error.status === 503) {
          errorMessage = "Servicio no disponible";
          errorDescription = "El servicio está temporalmente no disponible. Intenta más tarde.";
        } else {
          errorDescription = error.body?.detail || error.message || errorDescription;
        }
      } else if (error instanceof Error) {
        if (error.message.includes("Failed to fetch") || error.message.includes("NetworkError")) {
          errorMessage = "Error de conexión";
          errorDescription = "No se pudo conectar con el servidor. Verifica tu conexión y que el backend esté corriendo.";
        } else {
