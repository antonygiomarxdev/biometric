"""Visualiser for the pattern-first pipeline (Spike 03).

Extends the spike 02 visualizer with:
  - Panel showing the classified pattern + facing direction
  - The classification result as the title (not just inferred count)
  - Same NIST legend
"""
...
from pattern_first_detector import detect  # noqa: E402
from types_spike import FacingDirection, SingularityKind, Zone  # noqa: E402
...
def _draw_minutia(
    canvas: np.ndarray, x: int, y: int, angle: float, zone: Zone,
    confidence: float, is_termination: bool, ridge_trace_length: int,
    is_overlap: bool,
) -> None:
...
def _draw_singularity(
    canvas: np.ndarray, x: int, y: int, kind: SingularityKind,
) -> None:
...
def _make_grid(image: np.ndarray, result: Any) -> tuple[plt.Figure, dict[str, Any]]:
    ...
    classification = result.pattern_classification
    ...
    facing_str = f", facing: {classification.facing.value}" if classification.facing != FacingDirection.UNKNOWN else ""
    fig.suptitle(
        f"Spike 03: Pattern-First — {classification.pattern_type.value} "
        f"(conf={classification.confidence:.2f}{facing_str}) — "
        f"{result.num_minutiae} validated ({result.num_cores} cores, "
        f"{result.num_deltas} deltas)",
    ...
    if classification.pattern_type.value == "loop" and len(cores) >= 1 and len(deltas) >= 1:
        cv2.arrowedLine(
            canvas, (deltas[0].x, deltas[0].y), (cores[0].x, cores[0].y),
            (255, 255, 0), 2, line_type=cv2.LINE_AA, tipLength=0.15,
        )
    ...
    ax.text(
        0.05, 0.5,
        f"PATTERN CLASSIFICATION (Henry rule)\n\n"
        f"  type:         {classification.pattern_type.value}\n"
    ...
    ax.set_title("5. Pattern + overlap summary", fontsize=10)
    ax.axis("off")
    ...
    summary = (
        ...
        f"\n"
        f"SINGULARITIES (type-aware)\n"
        f"  raw cores:     {n_raw_cores}\n"
        f"  raw deltas:    {n_raw_deltas}\n"
        f"  after type:    {result.num_cores}-{result.num_deltas}\n"
        f"\n"
        ...
        if marker == "->":
            ...
            handles.append(
                plt.Line2D(
                    ...
                    markersize=12, label=label, linestyle="None",
                )
            )
    handles.append(
        plt.Line2D(
    ...
    leg = legend_ax.legend(
        handles=handles, loc="center", ncol=5, frameon=True,
...
def main(argv: list[str]) -> None:
    ...
    if not samples:
        print(f"No samples found. Checked: {SOCOFING_REAL}")
        sys.exit(1)

    print(f"Visualising {len(samples)} image(s) with the pattern-first detector.")
    ...
    for img_path in samples:
        ...
        plt.close(fig)
        print(
            f"     pattern={summary['pattern_type']:>12s}  "
            f"conf={summary['pattern_confidence']:.2f}  "
            f"facing={summary['facing']:>6s}  "
