# fingerprint/infrastructure/opencv/fingerprint_image_enhancer_impl.py
import math
...
class FingerprintImageEnhancerImpl:
    ...
    def __ridge_segment(self, img: np.ndarray):
        ...
        for i in range(0, new_rows, self.ridge_segment_blksze):
            for j in range(0, new_cols, self.ridge_segment_blksze):
                block = padded_img[i : i + self.ridge_segment_blksze][
                    :, j : j + self.ridge_segment_blksze
                ]

                stddevim[i : i + self.ridge_segment_blksze][
                    :, j : j + self.ridge_segment_blksze
    ...
    def __ridge_orient(self) -> None:
        ...
        denom = (
            np.sqrt(np.power(grad_xy, 2) + np.power((grad_x2 - grad_y2), 2))
            + np.finfo(float).eps
        )
        ...
        if self.orient_smooth_sigma:
            ...
            filter_gauss = gauss * gauss.T
            cos_2_theta = ndimage.convolve(
                cos_2_theta, filter_gauss
    ...
    def __ridge_freq(self):
        ...
        for i in range(0, rows - self.ridge_freq_blksze, self.ridge_freq_blksze):
            for j in range(0, cols - self.ridge_freq_blksze, self.ridge_freq_blksze):
                blkim = self._normim[i : i + self.ridge_freq_blksze][
                    :, j : j + self.ridge_freq_blksze
                ]
                blkor = self._orientim[i : i + self.ridge_freq_blksze][
                    :, j : j + self.ridge_freq_blksze
                ]

                freq[i : i + self.ridge_freq_blksze][
                    :, j : j + self.ridge_freq_blksze
    ...
    def __ridge_filter(self):
        ...
        for k in range(0, finalind_cols):
            ...
            img_block = norm_im[cur_r - sze : cur_r + sze + 1][
                :, cur_c - sze : cur_c + sze + 1
            ]
